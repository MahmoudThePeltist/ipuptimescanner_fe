    ...
    componentDidMount(){
        ...
        this.clientsService.getClients()
            .then(response => {
                ...
                response['data'] = response['data'].filter( client => client.type === 'cpe' );
        ...
        return (
            <Row className="h-100">
                <Col>
                    ...
                    {this.state.clientData ? this.state.clientData.map(client => 
                        client.status === 'up' ? 
                            <LitMarker key={ client.id } anchor={[  (1 * client.attributes.coordinates.x) , 
                                                                    (1 * client.attributes.coordinates.y)]}/>
                        : 
                            <DeadMarker key={ client.id } anchor={[(1 * client.attributes.coordinates.x) , 
                                                                    (1 * client.attributes.coordinates.y)]}/>
                    ) : false}
                    </Map>
                </Col>
