...
export class ClientFormComponent extends Component {
    ...
    componentDidMount(){
        ...
        if(this.props.client_id) {
            ...
            this.clientsService.getSpecificClient(this.props.client_id)
                .then( response => {
                    if(response['data'][0]){
                        ...
                        this.setState({
                            form_data: {
                                ...
                                type: form_data.type,
                                attributes: {
                                    coordinates: form_data.attributes? JSON.parse(form_data.attributes).coordinates : undefined,
                                },
                        ...
                        this.setState({message: {text: "No such user", type: "total_error"}});
                    }
                    console.log("Get Specific client response: ",response," State: ", this.state);
    ...
    handleInputChange(event) {
        ...
        {
            var coordinates = {...this.state.form_data.coordinates};
            coordinates[event.target.name] = event.target.value;

            this.setState({ form_data: {attributes: {coordinates: coordinates}}});
    ...
    render(){
        const { name, address, type, attributes, description } = this.state.form_data;
        return(
            <Card className="p-3">
                <div className="display-5 py-3">{ this.state.form_title }</div>

                { this.state.message.type !== 'total_error' ?
                    <Form className="mx-5" onSubmit={(e) => this.props.handleSubmit(e, this.state.form_data) }  id="add-client-form">
                        ...
                        <Row>
                            <Col>
                                <Form.Control   type="number" max="180.0000" min="-180.0000" step="0.00000001"
                                                ...
                                                name="x" value={ attributes.coordinates.x }
                                                onChange={this.handleInputChange} />
                            </Col>
                            <Col>
                                <Form.Control   type="number"  max="180.0000" min="-180.0000" step="0.00000001"
                                                placeholder="y coordinate" required
                                                name="y" value={ attributes.coordinates.y }
                                                onChange={this.handleInputChange} />
                            </Col>
                        </Row>
                    ...
                    <Button variant="primary" type="submit">
                        { this.state.form_title === 'Editing' ? 'Edit Client in System' : 'Add Client to System'}
                    </Button>
